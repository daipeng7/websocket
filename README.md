...
- **数据帧**
	...
	**opcode：** 定义了"Payloaddata"的解释，也就是定义了该数据是什么，如果不为定义内的值则连接中断。占四个位，可以表示0~15的十进制，或者一个十六进制。
...
- **解析数据帧**
	...
	因为WebSocket服务端接收到的数据是连续的数据帧，所以，需要对每一帧数据按照数据帧的结构进行解析，同时做边界的判断。边界判断的关键就是fin.
